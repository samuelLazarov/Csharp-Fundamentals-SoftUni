...
Dictionary<string, List<string>> planetsAction =
...
//dont use groups if you dont want to watch the same thing
...
for (int i = 0; i < messageNum; i++)
{
    ...
    string firstStage = DecryptFirstStage(messageEncrypted, key)
                                         .ToString();
    ...
    if (decrypted.Success)
    {
        ...
        if (attackType == "A")
        {
            planetsAction["Attacked"].Add(planetName);
        }
        else if (attackType == "D")
        {
            planetsAction["Destroyed"].Add(planetName);
        }
    }
}

foreach (var planet in planetsAction)
{
